...
int main(){
	...
	for(int w = 0; w <= t+maxval; w++){
		for(int c = 0; c < n; c++){
			for(int k = 1; k <= coins[c].s-used[w][c]; k++){

				if(w + k * coins[c].f <= t+maxval){

					if(dp[w + k * coins[c].f] > dp[w] + k){

						dp[w + k * coins[c].f] = dp[w] + k;

						used[w + k * coins[c].f][c] = used[w][c] + k;
					}
				}
			}
		}
