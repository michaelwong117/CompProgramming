...
int n, k, team[12001], dp[10001][1001], mx[10001][1001];
...
int solve(int prev, int cur){
	...
	if(cur - prev + 1 == k){
		dp[prev][cur-prev] = ((cur - prev + 1) * mx[prev][cur-prev]) + solve(cur+1, cur+1);
		return dp[prev][cur-prev];
	}
	else{
		dp[prev][cur-prev] = max((cur - prev + 1) * mx[prev][cur-prev] + solve(cur+1, cur+1), solve(prev, cur+1));
	}
