...
bool valid(pii start, pii finish){
	...
	map<pii, pii>::iterator it = roads.find(start);
	if(it != roads.end()){
		if(roads[start] == finish){
			// cout << "road exists " << start.fir << " " << start.sec << " "
			//  << finish.fir << " " << finish.sec << endl;
			return false;
		}
	}
...
int main(){
  ...
  for(int i = 0; i < R; i++){
  	int r1, c1, r2, c2;
  	fin >> r1 >> c1 >> r2 >> c2;
  	roads[pii(r1, c1)] = pii(r2, c2);
  	roads[pii(r2, c2)] = pii(r1, c1);
